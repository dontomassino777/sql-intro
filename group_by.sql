...
-- Find the average track length (in milliseconds) by album. 
-- Order the table by the averages.
SELECT album_id, AVG(milliseconds) FROM track
GROUP BY album_id
ORDER BY AVG(milliseconds);

-- Find a count of how many albums the artists with the ids 8 
-- and 22 have respectively. Use COUNT, WHERE/IN, and GROUP BY.
